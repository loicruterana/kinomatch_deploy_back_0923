...
const bookmarkedController = {
  ...
  addBookmarkedMovie: async (req, res) => {
    ...
    const existingMovie = await Bookmarked.findOne({
      where: { user_id: id.toString(), film_id: bookmarked.toString() },
    });
    ...
    if (!existingMovie && req.session.authorized) {
      try {
        ...
        res
          .status(200)
          .json({ message: 'bookmarked created', addMovieToBookmarked });
        return;
      } catch (error) {
        console.log(error);
        res.status(500);
  ...
  deleteBookmarkedMovie: async (req, res) => {
    ...
    try {
      ...
      const row = await Bookmarked.findOne({
